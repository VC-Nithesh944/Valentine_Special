...
import { Copy, Check, Sparkles, Wand2, Flower2, Gift, Gem, ArrowLeft, ExternalLink, Mail } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 relative overflow-hidden">
      <div className="absolute inset-0 z-0 opacity-20">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-[50vw] h-[50vw] bg-blue-500/20 rounded-full blur-[120px] animate-pulse" style={{ animationDelay: '2s' }} />
      </div>
        ...
        {!showGifts ? (
          ...
          >
            <div className="text-center mb-12">
              ...
              <h1 className="text-5xl md:text-6xl font-serif text-white mb-4">Personalize Your Story</h1>
            ...
            <div className="glass-card p-8 md:p-12 rounded-[2.5rem] space-y-8 border border-white/5 shadow-2xl">
              <div className="space-y-6">
                <div className="group">
                  <label className="text-[10px] tracking-[0.4em] uppercase text-white/30 mb-2 block font-medium group-focus-within:text-rose-400 transition-colors">Your Name</label>
                  <input
                    type="text"
                    value={yourName}
                ...
                <div className="group">
                  <label className="text-[10px] tracking-[0.4em] uppercase text-white/30 mb-2 block font-medium group-focus-within:text-rose-400 transition-colors">Your Partner's Name</label>
                  <input
                    type="text"
                    value={partnerName}
                ...
                <div className="group">
                  <label className="text-[10px] tracking-[0.4em] uppercase text-white/30 mb-2 block font-medium group-focus-within:text-rose-400 transition-colors">Your Email <span className="text-white/20 ml-2 normal-case tracking-normal">(To receive the confession)</span></label>
              ...
              {!generatedUrl ? (
                ...
                >
                  <div className="p-6 bg-rose-500/5 rounded-3xl border border-rose-500/10">
                    <p className="text-white/40 text-[10px] tracking-widest uppercase mb-4 font-medium">Your Unique Magic Link</p>
                    ...
                    <p className="mt-4 text-[9px] text-white/20 uppercase tracking-widest text-center">Copying will proceed to gifts automatically</p>
                  </div>
          ...
          >
            <div className="text-center mb-12">
              <button 
                ...
                className="mb-8 text-white/30 hover:text-white flex items-center gap-2 mx-auto text-[10px] uppercase tracking-widest transition-colors"
              >
                <ArrowLeft size={14} /> Back to Link
              </button>
              <h2 className="text-5xl md:text-6xl font-serif text-white mb-4">Complete the Surprise</h2>
              <p className="text-neutral-400 font-light tracking-wide max-w-xl mx-auto">
                Bro!! I'm planning something special for My Soulmate. If you loved the experience, help me make it real for my girl.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <motion.button
                whileHover={{ y: -10 }}
                onClick={() => openPayment(GIFTS.ROSE.amount, GIFTS.ROSE.name)}
                className="glass-card p-8 rounded-[2.5rem] border border-white/5 text-center flex flex-col items-center group transition-all duration-500 hover:border-rose-500/30"
              >
                ...
                <h3 className="text-2xl font-serif text-white mb-2">{GIFTS.ROSE.name}</h3>
                <p className="text-neutral-500 text-sm mb-6 leading-relaxed">
                  {GIFTS.ROSE.description}
                </p>
                <div className="mt-auto pt-6 border-t border-white/5 w-full">
                  <p className="text-rose-200 text-3xl font-serif mb-4">₹{GIFTS.ROSE.amount}</p>
                  <span className="text-[10px] text-white/30 uppercase tracking-[0.2em] font-bold group-hover:text-white transition-colors">Support Gift</span>
                </div>
              </motion.button>
              ...
              <motion.button
                whileHover={{ y: -10 }}
                onClick={() => openPayment(GIFTS.SILK.amount, GIFTS.SILK.name)}
                className="glass-card p-8 rounded-[2.5rem] border border-rose-400/20 text-center flex flex-col items-center relative group overflow-hidden transition-all duration-500 hover:border-rose-500/50"
              >
                <div className="absolute top-4 right-6 bg-rose-500 text-[9px] font-bold px-3 py-1 rounded-full uppercase tracking-widest text-white">Classic</div>
                <div className="w-16 h-16 rounded-full bg-rose-500/10 flex items-center justify-center mb-6 group-hover:bg-rose-500/20 transition-colors">
                  <Gift className="w-8 h-8 text-rose-400" />
                </div>
                <h3 className="text-2xl font-serif text-white mb-2">{GIFTS.SILK.name}</h3>
                <p className="text-neutral-500 text-sm mb-6 leading-relaxed">
                  {GIFTS.SILK.description}
                </p>
                <div className="mt-auto pt-6 border-t border-white/5 w-full">
                  <p className="text-rose-200 text-3xl font-serif mb-4">₹{GIFTS.SILK.amount}</p>
                  <span className="text-[10px] text-white/30 uppercase tracking-[0.2em] font-bold group-hover:text-white transition-colors">Support Gift</span>
                </div>
              </motion.button>
              ...
              <motion.button
                whileHover={{ y: -10 }}
                onClick={() => openPayment(GIFTS.SPECIAL.amount, GIFTS.SPECIAL.name)}
                className="glass-card p-8 rounded-[2.5rem] border border-white/5 text-center flex flex-col items-center group transition-all duration-500 hover:border-blue-500/30"
              >
                ...
                <h3 className="text-2xl font-serif text-white mb-2">{GIFTS.SPECIAL.name}</h3>
                <p className="text-neutral-500 text-sm mb-6 leading-relaxed">
                  {GIFTS.SPECIAL.description}
                </p>
                <div className="mt-auto pt-6 border-t border-white/5 w-full">
                  <p className="text-blue-200 text-3xl font-serif mb-4">₹{GIFTS.SPECIAL.amount}</p>
                  <span className="text-[10px] text-white/30 uppercase tracking-[0.2em] font-bold group-hover:text-white transition-colors">Support Gift</span>
                </div>
              </motion.button>
            </div>
      ...
      <footer className="absolute bottom-6 text-white/50 text-[9px] tracking-[1em] uppercase">
